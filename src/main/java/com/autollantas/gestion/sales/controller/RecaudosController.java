...
public class RecaudosController {
    ...
    private void aplicarFiltros() {
        ...
        filteredData.setPredicate(sale -> {
            if (!matchTexto(txtNumero.getText(), sale.getInvoiceNumber())) return false;
    ...
    private TableCell<Sale, LocalDate> crearCeldaFecha() {
    ...
    private TableCell<Sale, Double> crearCeldaMoneda(boolean esDeuda) {
        return new TableCell<>() {
    ...
    private void setupInteractionListeners() {
        tablaFacturas.setOnMouseClicked(event -> {
            if (event.getClickCount() == 2) {
                ...
                if (seleccionada != null) abrirHistorial(seleccionada);
            }
        });
        ...
        tablaFacturas.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.ENTER) {
                Sale seleccionada = tablaFacturas.getSelectionModel().getSelectedItem();
                if (seleccionada != null) { abrirHistorial(seleccionada); event.consume(); }
            }
        });
    }
    ...
    private void abrirModalRecaudo(Sale sale) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/autollantas/gestion/sales/views/FormularioRecaudo.fxml"));
            ...
            if (controller.isSaved()) cargarDatosDB();
    ...
    void btnVerDetalleClick(ActionEvent event) {
        Sale seleccion = tablaFacturas.getSelectionModel().getSelectedItem();
        if (seleccion != null) abrirHistorial(seleccion);
        else mostrarAlerta("Atención", "Seleccione un registro para ver el historial.");
    }

    private void abrirHistorial(Sale sale) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/autollantas/gestion/sales/views/HistorialRecaudos.fxml"));
            loader.setControllerFactory(springContext::getBean);
            Parent root = loader.load();
